...
- Sanitize after the cap, then parse, then normalize. Imported content only becomes document state after conversion into operations and `editor.apply(...)`. Before the final DOMPurify pass removes `<style>`, safe declarations from simple `.class` and `tag.class` rules are resolved onto matching elements; complex selectors and at-rules are ignored, and inline declarations win. Rules are collapsed by selector and property before elements are visited so work remains proportional to the capped source rather than multiplying matching elements by matching rules. The SEC3 style filter admits `color`, `background-color`, enumerated mark-bearing `font-weight`, `font-style`, `text-decoration`, and `text-decoration-line` values, and enumerated `text-align` keywords (`left` / `right` / `center` / `justify` / `start` / `end`), plus a validated HTML `align` attribute (`left` / `right` / `center` / `justify`). Inline mark styles map to Pen marks, including explicit resets such as `font-weight: normal` and `text-decoration: none`. Alignment maps to the default text blocks' `textAlignment` prop.
...
Workspace package at version `0.2.11`; intended usage is current-state but still evolving. HTML ingest is security-sensitive. JSON already defines Pen's explicit structured document contract.
...
- Do not make host delivery policy, general-purpose CSS inlining, or provider quirks part of this package. HTML ingest resolves only the documented closed subset needed to recover clipboard formatting.
