...
- Default inline-text blocks expose an optional `textAlignment` prop (`left`, `right`, `center`, `justify`, `start`, or `end`). Their HTML serializers emit the corresponding closed `text-align` declaration so pasted alignment survives later HTML interchange.
...
Workspace package at version `0.2.12`; intended usage is current-state but still evolving.
