...
- `src/editor/InlinePrompt.tsx` is the inline AI box from Input's composer: `⌘J` / `Ctrl+J` or the toolbar wand opens it on the current block. `target: "auto"` is the interesting bit — the library selection trigger only opens on a highlight; a caret is enough here. The box is inserted in the document flow before the target block (not overlaid), and `InlinePromptComposer.tsx` is Input's chrome: history, textarea, send arrow, Discard / Accept while a review is pending. `editor.css` hangs the popover look off the `data-pen-ai-inline-session-*` attributes.
