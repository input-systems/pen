...
- `useSuggestionMenu` / `resolveSuggestionMenuTarget` are thin bindings over the same functions in `@input/pen-core`. Matching uses logical offsets (N6): an inline atom is one unit, a query that contains an atom is rejected, and `startOffset` after an atom is the offset after that unit, not a `textContent()` index. `SuggestionMenu.Content` observes its rendered size and recomputes its anchored position when host content changes height, including when a host swaps equally sized item arrays with different row layouts. `removeInlineAtom` and `getInlineAtomAtOffset` are re-exported from `@input/pen-dom` on this package's root barrel.
...
- `Pen.Toolbar.Button` and `Pen.Toolbar.Toggle` compose a host `onClick` with their own action rather than letting it replace the action: the host handler runs first, and the action is skipped only when the button is disabled or that handler called `preventDefault()`. A Slot-style wrapper that merges its `onClick` onto the element (a tooltip trigger) therefore cannot silence the primitive. `renderAsChild` itself still lets `asChild` child props win.
- `useSelectionToolbar` reads the native DOM range rect for a selection inside one block and falls back to `resolveSelectionRect` from `@input/pen-dom`. For a selection that spans blocks it reads `resolveSelectionRect` first, because that is measured per block and the native range rect is the column width. It remeasures after React has laid out document commits, and `SelectionToolbar.Content` resolves live geometry again during its pre-paint positioning pass rather than trusting a cached commit measurement. Block alignment changes therefore cannot pair a new alignment with stale geometry. `SelectionToolbar.Content` positions against its `horizontalAlign`: `left` and `right` keep the corresponding selection edge fixed while `center`, the default, keeps the previous centred placement.
...
Workspace package at version `0.2.11`; intended usage is current-state but still evolving. This is still the main renderer the repo documents and validates most thoroughly.
