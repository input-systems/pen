...
- `useSelectionToolbar` reads the native DOM range rect for a selection inside one block and falls back to `resolveSelectionRect` from `@input/pen-dom`. For a selection that spans blocks it reads `resolveSelectionRect` first, because that is measured per block and the native range rect is the column width.
