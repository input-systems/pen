...
- `resolveBlockTextAlignment()` validates the shared `textAlignment` block prop before vanilla, React, or Vue renderers apply it to a block host.
...
Workspace package at version `0.2.12`; intended usage is current-state but still evolving. It is now a key architectural package because it proves the shared editing engine can outlive any single framework renderer.
